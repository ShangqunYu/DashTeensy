...
constexpr uint16_t kPort = 8007;  // udp port
constexpr int MAX_NUM_SAMPLES = 5000;


constexpr int NUM_BUSSES = 2;

int NUM_NODES[NUM_BUSSES] = {2, 2};
int BUS_IDS[NUM_BUSSES] = {0, 2};
...
EthernetUDP udp;
constexpr uint8_t TEENSY_IP[4] = { 192, 168, 0, 115 };
...
void setupCAN() {
  ...
  for(int i=0;i<NUM_BUSSES;++i){
    if(BUS_IDS[i]==0){
     ...
     if(BUS_IDS[i]==2){
      printf("can2 setup start\n");
      Can2.begin();
      printf("can2 begin done\n");
      Can2.setBaudRate(1000000);
      printf("can2 baudrate set\n");
      Can2.setMaxMB(NUM_TX_MAILBOXES + NUM_RX_MAILBOXES);
      printf("can2 set maxMB\n");
      Can2.setMBFilter(MB1, 0, 0x0);
      printf("can2 setMBfilter\n");
      Can2.enhanceFilter(MB1);
      printf("can2 enhance filter\n");
      Can2.enableMBInterrupts();
      printf("can2 enable interrupts\n");
      Can2.onReceive(canReceive3);
      printf("can2 onreceive\n");
...
void sendCANCMD() {
  ...
  for (int j = 0; j < NUM_BUSSES; j++) {
    for (int i = 0; i < NUM_NODES[j]; i++) {
      ...
      if (BUS_IDS[j] == 0) {
        Can0.write(msg[j]);
        printf("can 1 node %d: ",i);
        printCANCommand2(msg[j]);
        waitForResponse(j, i);
      } else if (BUS_IDS[j] == 1) {
      
        Can1.write(msg[j]);
        printf("can 2 node %d: ",i);
        printCANCommand2(msg[j]); 
        waitForResponse(j, i);
       
      } else if (BUS_IDS[j] == 2) {
        Can2.write(msg[j]);
        printf("can 3 node %d: ",i);
        printCANCommand2(msg[j]);
        waitForResponse(j, i);
      }
