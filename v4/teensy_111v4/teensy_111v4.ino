...
//            CAN Settings
//======================================================================
constexpr uint8_t NUM_BUSSES = 3;
uint8_t NUM_NODES[NUM_BUSSES] = {2, 2, 2};
uint8_t BUS_IDS[NUM_BUSSES]={0, 1, 2};
int8_t can_bus_idx[3]={0,1,2};
...
void setupCAN() {
  ...
  for(int i=0;i<NUM_BUSSES;++i){
    ...
    if(CanArray[BUS_IDS[i]]){
      ...
      FLEXCAN_MAILBOX MB_i = static_cast<FLEXCAN_MAILBOX>(static_cast<int>(MB1)+(NUM_RX_MAILBOXES)*i);
      can_i->begin();
      can_i->setBaudRate(1000000);
      can_i->setMaxMB(NUM_RX_MAILBOXES);
      // can_i->setMBFilter(REJECT_ALL);
      //setup mailboxes for receiving
      for (int j = 0; j<NUM_RX_MAILBOXES; j++){
        can_i->setMB((FLEXCAN_MAILBOX)(MB_i+j),RX,STD);
        can_i->onReceive((FLEXCAN_MAILBOX)(MB_i+j),canReceiveFuncs[BUS_IDS[i]]);
        // can_i->setMBFilter((FLEXCAN_MAILBOX)(MB_i+j), j); // Set mailboxes to receive messages  
        can_i->setMBFilter((FLEXCAN_MAILBOX)(MB_i+j),0,0x0);
        can_i->enhanceFilter((FLEXCAN_MAILBOX)(MB_i+j));
        DEBUG_PRINT("set up MB %d for receiving\n",(MB_i+j));
      }
      can_i->enableMBInterrupts();

      DEBUG_PRINT("receiving mailboxes are setup for bus %d\n",BUS_IDS[i]);
      //setup mailboxes for transmitting
      // for (int j = NUM_RX_MAILBOXES; j<(NUM_TX_MAILBOXES + NUM_RX_MAILBOXES); j++){
      //   can_i->setMB((FLEXCAN_MAILBOX)(MB_i+j),TX,STD);
      //   can_i->setMBFilter((FLEXCAN_MAILBOX)(MB_i+j), j- NUM_RX_MAILBOXES);
      //   can_i->enhanceFilter((FLEXCAN_MAILBOX)(MB_i+j));
      //   DEBUG_PRINT("set up MB %d for transmitting\n",(MB_i+j));
      // }
      // can_i->mailboxStatus();
      // DEBUG_PRINT("transmitting mailboxes are setup for bus %d\n",BUS_IDS[i]);
      can_i->mailboxStatus();
      can_i->distribute();
